...
**Prerequisites:** Exponential trial solutions, complex-conjugate pairs, and the spring-mass-damper transfer function from [Lecture 1, §4.5](lecture_0.0.8_student_lecture_1.md#section-4-5) and [§5](lecture_0.0.8_student_lecture_1.md#section-5), together with elementary integration.

**Where this fits:** Read this addendum before [Lecture 1, §15: Transition to the Laplace transform](lecture_0.0.8_student_lecture_1.md#section-15). It develops the signal representation used there and prepares for the switched-heater calculation in [Lecture 1, §17.1](lecture_0.0.8_student_lecture_1.md#section-17-1).
...
In [Lecture 1, §5](lecture_0.0.8_student_lecture_1.md#section-5), the forced model
...
**Answer:** $\cos^3\omega t=\tfrac34\cos\omega t+\tfrac14\cos3\omega t$. A frequency that was never in the input appears in the output. The exponentials no longer pass through separately, so no single complex number per frequency can describe the system. Linearity is what keeps them apart. A transfer function can still describe a specified linearization about an operating point.
...
For a real model, $G(-j\omega)=\overline{G(j\omega)}$. The two terms are therefore conjugates, and their sum is real. This is the [Lecture 1, §4.5](lecture_0.0.8_student_lecture_1.md#section-4-5) argument again, now applied to a forced response. Write $G(j\omega)=|G(j\omega)|e^{j\angle G(j\omega)}$:
...
Pair positive $n$ with $-n$ exactly as in [§2.2](#section-2-2). For positive odd $n$, $U_ne^{jn\omega_0t}+U_{-n}e^{-jn\omega_0t}=\dfrac{2}{\pi n}\sin n\omega_0t$, so
...
The subscript "ss" means steady state. As in [Lecture 1, §5](lecture_0.0.8_student_lecture_1.md#section-5), the complete response adds the free modes, and they decay here because $m>0$, $k>0$ and $c>0$.
...
Indeed, $(D^2+25)(t\cos5t)=-10\sin5t$. The same-exponential trial fails because $G(j5)$ is undefined. The resonant response grows without bound; there is no bounded periodic steady state, and homogeneous oscillations no longer decay.
...
For **each fixed $n$**, $U_n\to1/T$ as $\varepsilon\to0$. For a finite pulse, the coefficients are approximately $1/T$ only when $|n\omega_0\varepsilon|\ll1$; higher-frequency coefficients decay and have zeros. The limit is not uniform over all harmonics.
...
This is the pair used in [Lecture 1, §17.1](lecture_0.0.8_student_lecture_1.md#section-17-1) to switch on the heater.
...
This is the inversion formula in [Lecture 1, §15](lecture_0.0.8_student_lecture_1.md#section-15), interpreted by symmetric frequency truncation. At a jump it recovers the midpoint of the two one-sided limits; the step's value at zero is therefore $1/2$ under inversion, regardless of its assigned endpoint value.
...
For the impulse row, use the $0^-$ convention of [Lecture 1, §15](lecture_0.0.8_student_lecture_1.md#section-15): $\mathcal L\{\delta(t)\}=\int_{0^-}^\infty\delta(t)e^{-st}\,dt=1$, including the impulse at zero. For ordinary signals, writing 0 as the lower limit has the same effect.
...
Partial fractions and transform pairs recover the complete switched response, including the transient. Nonzero initial conditions contribute additional terms, as shown in [Lecture 1, §17](lecture_0.0.8_student_lecture_1.md#section-17).
