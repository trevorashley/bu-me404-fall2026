...
**Prerequisites:** [L1](lecture_ch3_L1_laplace-and-transfer-functions_student.md), [L2](lecture_ch3_L2_block-diagrams-and-pole-locations_student.md), and [L3](lecture_ch3_L3_specifications-and-zeros_student.md).
...
**Worked check:** At $K=K_I=1$, $T(s)=(s+1)/(s+1)^3=1/(s+1)^2$, with unit final value. At $K=10,K_I=5$, the roots are approximately $-0.462,-1.269\pm3.036j$, and the zero is at $-0.5$. The imaginary part is approximately 3.036, correcting the textbook’s value near 3.3. Treat $K_I=0$ separately, as explained below.
