...
**Row by row.** The first two rows are the alternate coefficients: $1,3,1,4$ from $s^6,s^4,s^2,s^0$ and $4,2,4,0$ from $s^5,s^3,s^1$. Each later entry is (first-column pivot above × entry up two and one column right − entry up two in the first column × entry above and one column right) ÷ pivot above:
...
The last entry always equals the constant coefficient $a_n$, which is a quick check. The $s^4$ row contains a zero, but not in its first column, so the ordinary array is still defined.
...
*The free test first.* The coefficients require $K-6>0$ and $K>0$, so $K>6$ is necessary. Routh will tighten this.
...
The first inequality gives $4K>30$, so $K>7.5$. That implies $K>0$ and the necessary condition $K>6$. The interval $6<K\le7.5$ is exactly the region where the free test passes but the system is not stable. For $0<K<7.5$, the first column reads $+,+,-,+$: two sign changes, so two RHP roots.
